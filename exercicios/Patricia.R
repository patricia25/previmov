...
source("C:/Users/user/previmov//dados_novos.R") #dados salvos na variavel aptos
#Com a função unique retira-se da amostra todas as observações que há dupla contagem, observando a amostra, verifica-se que pode haver a mesma observação mais de uma vez, foi considerado observação igual aqueles que apresentam as mesmas características.
...
# aqui foram incluidos as interações are*vaga e a interação area/quarto.
...
# A interação arevaga não trouxe nenhuma melhora para o modelo, por isso não foi mantida na formula diferente da interação areaquarto que trouxe alguma melhora ao modelo
...
#diminuindo as repetições para diminuir o  custo computacional
...
#com a utilização da função unique retira-se as possiveis duplas contagens e também foi implementado novas interações e diminuiu o custo computacional do modelo.
